...
RUN set -x \
    ...
    && apt-get install --no-install-recommends --no-install-suggests -y net-tools iproute2 ca-certificates linux-headers-$(uname -r) git gcc make cmake libpcre3-dev libssl-dev liblua5.1-0-dev
...
RUN set -x \
    ...
    && cd "build" \
    && cmake -DBUILD_DRIVER=FALSE -DKDIR=/usr/src/linux-headers-$(uname -r)-generic -DCMAKE_INSTALL_PREFIX=/usr/local -DCMAKE_BUILD_TYPE=Release -DLOG_PGSQL=FALSE -DSHAPER=FALSE -DRADIUS=FALSE -DNETSNMP=FALSE -DLUA=TRUE .. \
    && make \
    && make install \
    && cd / \
    && rm -rf ${build_dir} \
